...
def npl [...args ] {
    nix profile list --json ...$args | from json | get elements | transpose | rename "name" "details"
}

# Reverses a transpose by turning a table into
# a record with column 
def transcribe [key_column, value_column] {
    $in 
    | each {|x| {($x | get $key_column): ($x | get $value_column)}}
    | reduce {|it, acc| $it | merge $acc}
}

# nushell wrapper for `exiftool`
def exif [...args] {
    exiftool -json ...$args | from json
}
